...
def clear_mem():
  # clear VRAM/GPU memory -- compatible with old and new JAX
  try:
    try:
...
def tree_map(fn, x):
  # jax.tree_util.tree_map was removed in newer JAX
      ...
      if v is not None:
        if k in d:
          if isinstance(v, dict):
            ...
            d[k] = tree_map(lambda y: type(y)(v), d[k])
          else:
            d[k] = type(d[k])(v)
        else:
